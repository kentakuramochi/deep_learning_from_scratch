...
class LSTM:
    ...
    def forward(self, x, h_prev, c_prev):
        ...
        # Aggregate affine transformations for each gate
        A = np.dot(x, Wx) + np.dot(h_prev, Wh) + b

        # Slice
        f = A[:, :H]  # For the forget gate
        g = A[:, H:2*H]
        i = A[:, 2*H:3*H]  # For the input gate
        o = A[:, 3*H:]  # For the output gate
        ...
        self.cache = (
            x, h_prev, c_prev, i, f, g, o, c_next
        )
    ...
    def backward(self, dh_next, dc_next):
        ...
        # Backward based on computation graph...
        ...
        # to aggregate matmul for backward propagation
...
class TimeLSTM:
    ...
    def forward(self, xs):
        ...
        # Initialize memory cell and hidden state vector
        if not self.stateful or self.h is None:
            self.h = np.zeros((N, H), dtype="f")
...
class TimeDropout:
    """ Dropout layer unit for T time series
    """
    ...
    def forward(self, xs):
        if self.train_flg:
            # Create a mask by probability while training
